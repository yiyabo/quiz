...
def evaluate_submission(submission_file, labels_file='test_labels_cci.csv'):
    """评估提交文件"""
    ...
    # 加载真实标签
    print("加载测试集真实标签...")
    labels = load_csv_as_dict(labels_file, ['source', 'traget'])
    print(f"  测试集样本数: {len(labels)}")

    # 加载学生提交
    print(f"\n加载学生提交: {submission_file}")
    try:
        submissions = load_csv_as_dict(submission_file, ['source', 'traget'])
    ...
    # 验证格式
    print("\n验证提交格式...")
    required_cols = ['source', 'traget', 'label']
